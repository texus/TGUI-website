---
layout: page
title: Minimal code with SDL_GLES2 backend
breadcrumb: minimal code with sdl_gles2
---

The following is the minimal code you need for an application using TGUI with the SDL\_GLES2 backend. The code contains some comments with explanations, but some extra information and some alternatives are also provided below the example code.
```c++
...
To draw all widgets in the gui, you need to call `gui.draw()` once per frame. All widgets are drawn at once, the SDL\_GLES2 backend currently doesn't provide a way to render OpenGL contents inbetween TGUI widgets (without creating a custom widget).
